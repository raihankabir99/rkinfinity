  ...
  return (
    <div className="space-y-4">
      ...
      <label className="block text-sm">
        ...
        <input type="number" value={traffic} onChange={(e) => setTraffic(Number(e.target.value) || 0)}
          className="w-full glass rounded-lg px-3 py-2 mt-1 outline-none" />
      </label>
      <NeonButton onClick={() => setShow(true)} disabled={!url}>Estimate Worth</NeonButton>
      {data && (
        <>
          <div className="grid grid-cols-3 gap-3 text-center">
            <div className="glass rounded-lg p-4"><div className="text-2xl font-black text-gradient">${data.monthly}</div><div className="text-xs uppercase text-muted-foreground mt-1">/ month</div></div>
            <div className="glass rounded-lg p-4"><div className="text-2xl font-black text-gradient">${data.annual.toLocaleString()}</div><div className="text-xs uppercase text-muted-foreground mt-1">/ year</div></div>
            <div className="glass rounded-lg p-4"><div className="text-2xl font-black text-gradient">${data.worth.toLocaleString()}</div><div className="text-xs uppercase text-muted-foreground mt-1">Site worth</div></div>
          </div>
          <div className="glass rounded-lg p-4 h-64">
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                <Pie data={data.breakdown} cx="50%" cy="50%" outerRadius={80} innerRadius={45} dataKey="value">
          ...
          <PdfButton onClick={() => downloadPdf(`Website Worth - ${url}`, [
            `URL: ${url}`, `Monthly visitors: ${traffic}`, `Monthly revenue: $${data.monthly}`,
            `Annual revenue: $${data.annual}`, `Estimated worth: $${data.worth}`,
            ...data.breakdown.map((b) => `${b.name}: $${b.value}`),
          ])} />
          <p className="text-xs text-muted-foreground italic">Realistic estimate based on traffic & monetization heuristics.</p>
        </>
      )}
    </div>
  );
...
export function HumanizerTool() {
  ...
  const humanize = async () => {
    ...
    result = result.replace(/\bdo not\b/gi, "don't").replace(/\bcannot\b/gi, "can't").replace(/\bit is\b/gi, "it's").replace(/\byou are\b/gi, "you're");
    setOut(result);
    setLoading(false);
  };
  ...
  return (
    <div className="space-y-3">
      <textarea value={text} onChange={(e) => setText(e.target.value)} rows={6} placeholder="Paste AI-generated text..."
        className="w-full glass rounded-lg px-3 py-2 text-sm outline-none" />
      <NeonButton onClick={humanize} disabled={!text || loading}>Humanize</NeonButton>
      {loading && <Spinner label="Rewriting in a human voice..." />}
      {out && (
        <>
          <div className="glass rounded-lg p-4 text-sm leading-relaxed whitespace-pre-wrap">{out}</div>
          <button onClick={() => navigator.clipboard.writeText(out)} className="text-xs text-primary hover:underline">Copy</button>
...
export function RankBattleTool() {
  ...
  return (
    <div className="space-y-4">
      ...
      <NeonButton onClick={() => setShow(true)} disabled={!a || !b}>⚔ Battle!</NeonButton>
      {show && (
        <>
          <div className="glass rounded-lg p-4 text-center">
            ...
            <div className="text-xs text-muted-foreground mt-1">{winsA} – {winsB}</div>
          </div>
          ...
          <PdfButton onClick={() => downloadPdf(`Rank Battle - ${a} vs ${b}`, [

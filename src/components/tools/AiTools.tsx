...
export function BlogIntroTool() {
  ...
  return (
    <div className="space-y-3">
      ...
      <NeonButton onClick={run} disabled={!topic || loading}>Generate 3 Variations</NeonButton>
      ...
      {out.map((o, i) => (
        <div key={i} className="glass rounded-lg p-4 text-sm leading-relaxed">
          ...
          {o}
          <button onClick={() => navigator.clipboard.writeText(o)} className="block mt-2 text-xs text-primary hover:underline">Copy</button>
        </div>
      ))}
    </div>
  );
  ...
  return (
    <div className="space-y-3">
      <input value={topic} onChange={(e) => setTopic(e.target.value)} placeholder="Video topic..."
        className="w-full glass rounded-lg px-3 py-2 text-sm outline-none" />
      <NeonButton onClick={run} disabled={!topic || loading}>Generate Script</NeonButton>
      {loading && <Spinner label="Writing script..." />}
      {out && (
        <pre className="glass rounded-lg p-4 text-xs whitespace-pre-wrap font-sans leading-relaxed">{out}</pre>
      )}
    </div>
  );
}
...
export function SummarizerTool() {
  ...
  return (
    <div className="space-y-3">
      <textarea value={text} onChange={(e) => setText(e.target.value)} rows={6} placeholder="Paste long text..."
        className="w-full glass rounded-lg px-3 py-2 text-sm outline-none" />
      <NeonButton onClick={run} disabled={!text || loading}>Summarize</NeonButton>
      {loading && <Spinner />}
      {out.length > 0 && (
        <div className="glass rounded-lg p-4 space-y-1.5 text-sm">{out.map((o, i) => <div key={i}>{o}</div>)}</div>
      )}
    </div>
  );
}
...
export function HashtagTool() {
  ...
  const run = () => {
    ...
    const sets: Record<string, string[]> = {
      instagram: [`#${base}`, `#${base}love`, `#${base}gram`, `#${base}daily`, `#instagood`, `#explore`, `#viral`, `#trending`, `#${base}life`, `#${base}community`, `#photooftheday`, `#reels`],
      twitter: [`#${base}`, `#${base}Tips`, `#${base}2026`, `#TechTwitter`, `#BuildInPublic`],
      linkedin: [`#${base}`, `#${base}Strategy`, `#Leadership`, `#GrowthMindset`, `#${base}Insights`, `#FutureOfWork`],
      tiktok: [`#${base}`, `#${base}tok`, `#fyp`, `#foryoupage`, `#viral`, `#${base}challenge`, `#trending2026`],
    };
  ...
  return (
    <div className="space-y-3">
      <div className="flex gap-2">
        ...
        <NeonButton onClick={run} disabled={!topic}>Generate</NeonButton>
      </div>
      {out.length > 0 && (
        <>
          <div className="glass rounded-lg p-4 flex flex-wrap gap-2 text-sm text-primary">
            {out.map((t) => <span key={t} className="font-mono">{t}</span>)}
          </div>
          <button onClick={() => navigator.clipboard.writeText(out.join(" "))} className="text-xs text-primary hover:underline">Copy all</button>
        </>
      )}
    </div>
  );

...
export function PageSpeedTool() {
  ...
  const run = async () => {
    ...
    try {
      ...
      if (res.status === 429) {
        throw new Error(
          "Google PageSpeed is rate-limiting (429). Please wait ~30 seconds and try again.",
        );
      }
  ...
  return (
    <div className="space-y-4">
      <div className="flex gap-2">
        ...
        />
        <NeonButton onClick={run} disabled={!url || loading}>
          Analyze
        </NeonButton>
      </div>
      {err && (
        ...
        <>
            ...
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                <Pie
                  ...
                  dataKey="value"
                  label={(p) =>
                    `${(p as { name?: string }).name ?? ""}: ${(p as { value?: number }).value ?? ""}`
                  }
                >
                  {chartData.map((_, i) => (
          ...
          {(data.lcp || data.cls) && (
            <div className="glass rounded-lg p-4 text-xs space-y-1 font-mono">
              {data.lcp && (
                <div>
                  LCP: <span className="text-primary">{data.lcp}</span>
                </div>
              )}
              {data.cls && (
                <div>
                  CLS: <span className="text-primary">{data.cls}</span>
                </div>
              )}
            </div>
          )}
            ...
            onClick={() =>
              downloadPdf(
                ...
                [
                  ...
                  `Best Practices: ${data.bp}`,
                  data.lcp ? `LCP: ${data.lcp}` : "",
                  data.cls ? `CLS: ${data.cls}` : "",
                ].filter(Boolean),
              )
            }
          />
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-col sm:flex-row gap-2">
        ...
        <NeonButton onClick={run} disabled={loading || !q}>
          Suggest
        </NeonButton>
      </div>
      ...
      {results.length > 0 && (
        <div className="grid sm:grid-cols-2 gap-2">
          {results.map((r) => (
            <div
              key={r}
              className="glass rounded-lg px-4 py-2.5 text-sm hover:border-primary/40 transition flex items-center justify-between"
            >
              <span>{r}</span>
...
export function SeoAuditTool() {
  ...
  const run = async () => {
    ...
    try {
      ...
      setErr(
        e instanceof Error
          ? `Failed to fetch — ${e.message}. Try again or check the URL.`
          : "Failed to fetch",
      );
    } finally {
      setLoading(false);
    ...
    <div className="space-y-4">
      <div className="flex gap-2">
        <input
          ...
          className="flex-1 glass rounded-lg px-4 py-2.5 text-sm outline-none focus:border-primary/50"
        />
        <NeonButton onClick={run} disabled={loading || !url}>
          Audit
        </NeonButton>
      </div>
      ...
      {data && (
        <div className="space-y-3 text-sm">
          ...
          <AuditRow
            label="Title"
            value={data.title}
            ok={data.title !== "(missing)" && data.title.length < 70}
          />
          <AuditRow
            label="Meta Description"
            value={data.desc}
            ok={data.desc !== "(missing)" && data.desc.length < 160}
          />
          <AuditRow
            label={`H1 (${data.h1.length})`}
            value={data.h1.join(" · ") || "(none)"}
            ok={data.h1.length === 1}
          />
          <AuditRow
            label={`H2 (${data.h2.length})`}
            value={data.h2.slice(0, 5).join(" · ") || "(none)"}
            ok={data.h2.length > 0}
          />
          <AuditRow
            label={`H3 (${data.h3.length})`}
            value={data.h3.slice(0, 5).join(" · ") || "(none)"}
            ok={true}
          />
          <PdfButton
            onClick={() =>
              downloadPdf(`SEO Audit - ${url}`, [
                `URL: ${url}`,
                `Status: ${data.status}`,
                `Title: ${data.title}`,
      ...
      const checks = await Promise.all(
        unique.map(async (link) => {
          try {
            ...
            return { link, status: "ERR" as const };
          }
        }),
      );
      setResults(checks);
        ...
        <div className="space-y-1.5 max-h-96 overflow-y-auto">
          {results.map((r) => {
            ...
            return (
              <div
                key={r.link}
                className="glass rounded-lg px-3 py-2 text-xs flex items-center justify-between gap-3"
              >
                <span className="truncate font-mono">{r.link}</span>
                <span
                  className={`shrink-0 font-bold ${broken ? "text-destructive" : "text-primary"}`}
                >
                  {r.status}
                </span>
              </div>
...
export function SitemapTool() {
  ...
  return (
    <div className="space-y-3 text-sm">
      ...
      />
      <pre className="glass rounded-lg p-4 text-xs font-mono whitespace-pre-wrap max-h-72 overflow-auto">
        {xml}
      </pre>
  ...
  return (
    <div className="space-y-3 text-sm">
      ...
      <div className="text-xs text-muted-foreground">
        Total words: <span className="text-primary font-bold">{words.length}</span>
      </div>
      {chart.length > 0 && (
      ...
      <div className="space-y-1.5">
        {top.map(([w, c]) => (
          <div
            key={w}
            className="glass rounded-lg px-3 py-2 flex items-center justify-between text-xs"
          >
      ...
      {top.length > 0 && (
        <PdfButton
          onClick={() =>
            downloadPdf("Keyword Density Report", [

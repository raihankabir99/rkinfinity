...
import { MessageSquare, Loader2, ShieldCheck, ArrowLeft, Bot, User as UserIcon, Send, Bell, BellOff } from "lucide-react";
...
function ChatsPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const { data: roleRows } = await supabase
        ...
        .eq("user_id", sess.session.user.id).eq("role", "admin");
      if (!mounted) return;
  ...
  const loadAll = async () => {
    const [{ data: m }, { data: u }] = await Promise.all([
      ...
      supabase.from("chat_users").select("*").order("last_seen_at", { ascending: false }).limit(500),
    ]);
    ...
    const ch = supabase
      ...
      .on("postgres_changes",
        ...
        (payload) => {
          ...
          if (row.role === "user" && notif && !focusRef.current && "Notification" in window && Notification.permission === "granted") {
  ...
  const enableNotifications = async () => {
    if (!("Notification" in window)) { toast.error("Notifications unsupported"); return; }
    if (Notification.permission === "granted") { setNotif(true); toast.success("Notifications on"); return; }
    const p = await Notification.requestPermission();
    if (p === "granted") { setNotif(true); toast.success("Notifications enabled"); }
  ...
  const toggleMode = async () => {
    ...
    const { error } = await supabase.from("chat_users").update({ mode: nextMode }).eq("session_id", active);
    if (error) toast.error(error.message);
    else { setUsers((u) => ({ ...u, [active]: { ...activeUser, mode: nextMode } })); toast.success(`Switched to ${nextMode.toUpperCase()} mode`); }
  };
  ...
  const sendReply = async () => {
    ...
    const { error } = await supabase.from("chat_messages").insert({
      session_id: active, role: "admin", content: text,
    });
    if (error) toast.error(error.message);
  };

  if (authChecking) {
    return <PageShell><div className="flex min-h-[60vh] items-center justify-center"><Loader2 className="h-6 w-6 animate-spin text-primary" /></div></PageShell>;
  }
  if (!isAdmin) {
    return <PageShell><div className="mx-auto max-w-md px-4 py-20 text-center"><ShieldCheck className="mx-auto mb-4 h-10 w-10 text-primary" /><h1 className="text-2xl font-bold">Admin access required</h1><Link to="/" className="mt-6 inline-block text-primary hover:underline">← Back home</Link></div></PageShell>;
  }
      ...
      <section className="mx-auto max-w-7xl px-4 py-10">
        ...
        <div className="mb-6 flex items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-sm text-muted-foreground">{sessions.length} sessions · {msgs.length} messages · realtime</p>
          </div>
          <button
        ...
        <div className="grid gap-4 lg:grid-cols-[320px_1fr]">
          <div className="glass rounded-2xl border border-white/5 p-3 max-h-[70vh] overflow-y-auto">
            {sessions.length === 0 && <p className="py-8 text-center text-xs text-muted-foreground">No conversations yet.</p>}
            {sessions.map((s) => {
              const u = users[s.sid];
              const isReturning = u && new Date(u.created_at).getTime() < Date.now() - 24 * 60 * 60 * 1000;
              return (
                <button
                  key={s.sid}
                  onClick={() => setActive(s.sid)}
                  className={`w-full text-left rounded-xl px-3 py-2.5 mb-1 transition ${
                    active === s.sid ? "bg-primary/15 border border-primary/40" : "hover:bg-white/5 border border-transparent"
                  }`}
                >
                  <div className="flex items-center justify-between gap-2">
                    ...
                    <span className="text-[10px] text-muted-foreground shrink-0">{fmtTime(s.last.created_at)}</span>
                  </div>
                  <p className="text-xs text-foreground/80 mt-1 truncate">{s.last.content || "—"}</p>
                  <div className="flex items-center gap-2 mt-1">
                    <span className="text-[10px] text-muted-foreground">{s.msgs.length} msgs</span>
                    {u?.city && <span className="text-[10px] text-muted-foreground">· {u.city}{u.country ? `, ${u.country}` : ""}</span>}
                    {u?.device && <span className="text-[10px] text-muted-foreground">· {u.device}</span>}
                    <span className={`ml-auto text-[10px] uppercase tracking-wider px-1.5 py-0.5 rounded ${
                      u?.mode === "manual" ? "bg-accent/20 text-accent" : "bg-primary/15 text-primary"
                    }`}>{u?.mode ?? "ai"}</span>
          ...
          <div className="glass rounded-2xl border border-white/5 flex flex-col max-h-[70vh]">
            {!active && <p className="py-12 text-center text-sm text-muted-foreground">Select a session to view the conversation.</p>}
            {active && (
              <>
                <div className="border-b border-white/5 p-4 flex items-center justify-between">
                  <div>
                    <div className="text-sm font-semibold">{activeUser?.user_name ?? "Anonymous visitor"}</div>
                    <div className="text-[10px] text-muted-foreground font-mono">{active.slice(0, 16)}…</div>
                  </div>
                  <button
                    onClick={toggleMode}
                  ...
                  >
                    {activeUser?.mode === "manual" ? "Manual mode (you reply)" : "AI mode (auto-reply)"}
                  </button>
                </div>
                  ...
                  {activeMsgs.map((m) => (
                    <div key={m.id} className={`flex gap-2 ${m.role === "user" ? "" : "justify-end"}`}>
                      ...
                      <div className={`rounded-2xl px-3 py-2 text-sm max-w-[80%] whitespace-pre-wrap ${
                        ...
                        {m.content}
                        <div className="text-[10px] text-muted-foreground mt-1">{fmtTime(m.created_at)} · {m.role}</div>
                      </div>
                      {m.role !== "user" && (
                        <span className={`grid h-7 w-7 shrink-0 place-items-center rounded-full ${
                          m.role === "admin" ? "bg-accent/20 text-accent" : "bg-primary/15 text-primary"
                        }`}>
                          <Bot size={12} />
                        </span>
                      )}

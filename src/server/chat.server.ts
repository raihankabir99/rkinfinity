...
const NAV_INTENTS = [
  ...
  {
    rx: /\b(kpi|stats|analytics|performance|metrics)/i,
...
const PROJECT_TRACKING_RX =
  /\b(track|status|progress|update|kaj|kototok|amader|কাজ|কতটুক|koto|dur|kivabe|cholche|obostha)\b/i;
...
async function runChatUnsafe({ messages, session_id, user_name }: ChatInput, context: any) {
  ...
  if (pid || PROJECT_TRACKING_RX.test(lastUser)) {
    if (pid) {
      ...
      return {
        content: "I can help with that. What is the project ID you would like me to check?",
  ...
  if (nav && /show|where|find|page|link|take me|go to|see|open/i.test(lastUser)) {
    return {
      content: `Sure! Check out the **${nav.label}** page → [${nav.path}](${nav.path}) ✨`,
      source: "nav" as const,
    };
  }

  // 4. Trained answers
  const { data: trainedData } = await supabaseAdmin
    .from("bot_training")
  ...
  if (trainedData) {
    ...
    for (const row of trainedData as Array<{ question: string; answer: string }>) {
      ...
      if (q.includes(k) || k.includes(q))
        return { content: row.answer, source: "trained" as const };
    }
  }

  // 5. Knowledge base search
  const words = lastUser
    .toLowerCase()
    .split(/\W+/)
    .filter((w) => w.length > 3)
    .slice(0, 5);
  ...
  if (!apiKey) {
    ...
    return {
      content:
        "Sorry, I'm having a little trouble thinking right now. Please try again in a moment.",
      source: "ai" as const,
    };
  }
  ...
  const result = await ai.models.generateContent({
    ...
    contents: history,
    config: {
      systemInstruction: `You are RK's friendly assistant for rkInfinity — RK is an SEO Specialist, Digital Marketer, Content Creator and Story Writer. Be warm, concise, and helpful. You can suggest these pages: /services, /tools, /blog, /about, /contact. Keep replies short (2-4 sentences) and use markdown sparingly.`,
    },
  });
  return { content: result.text || "", source: "ai" as const };
}
